...
export default function Header({ title, breadcrumbs = [], profile }) {
  return (
    <header className="h-16 border-b border-slate-200 bg-white/80 backdrop-blur-xl flex items-center justify-between px-4 sticky top-0 z-20">
      
      {/* LEFT: LOGO FOR MOBILE / BREADCRUMBS FOR DESKTOP */}
      <div className="flex items-center">
        {/* Mobile Logo (Now shown on all screens) */}
        ...
        {/* Desktop Breadcrumb & Title (Hidden globally to match mobile layout) */}
      ...
      <div className="flex items-center gap-3">
        {/* NOTIFICATIONS BELL */}
        <button className="relative w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 hover:border-[#004b87]/50 flex items-center justify-center text-slate-500 hover:text-[#0f294a] transition-all duration-300">
          <Bell size={16} />
          <span className="absolute top-1.5 right-2 w-2 h-2 rounded-full bg-[#004b87] ring-4 ring-white"></span>
        </button>
 
        {/* PROFILE CHIP */}
        <div className="flex items-center gap-2 bg-slate-55/80 hover:bg-slate-100/80 border border-slate-200 hover:border-slate-300 px-2.5 py-1.5 rounded-xl transition-all duration-300 group cursor-pointer relative">
          <div className="w-7 h-7 rounded-lg bg-[#f0f4ff] flex items-center justify-center text-[#004b87] font-bold overflow-hidden shadow-sm">
            <User size={14} />
          </div>
          <div className="hidden sm:flex flex-col pr-1">
            <span className="text-[11px] font-bold text-[#0f294a] group-hover:text-[#004b87] transition-colors">
