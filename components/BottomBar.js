  ...
  const directItems = [
    {
      ...
      href: '/dashboard',
      icon: (active) => (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className={`w-5.5 h-5.5 transition-all duration-300 ${active ? 'text-[#c69818] scale-110' : 'text-slate-400'}`}>
  ...
  const drawerItems = [
    {
      ...
      href: '/nup',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6 text-[#c69818]">
      ...
      <div className="fixed bottom-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-xl border-t border-slate-200 z-40 lg:hidden flex justify-around items-center px-4 pb-safe shadow-2xl">
            ...
            return (
              <Link key={item.href} href={item.href} className="flex flex-col items-center justify-center py-1 group">
                <div className="relative">
                  {item.icon(active)}
                  {active && (
                    <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse" />
                  )}
                </div>
        ...
        <div className="relative flex justify-center items-center w-1/5">
          <button
            ...
            className={`absolute -top-7 w-14 h-14 rounded-full bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 text-slate-950 flex items-center justify-center shadow-lg shadow-amber-500/20 active:scale-90 transition-all duration-200 border-4 border-white focus:outline-none z-50 ${isDrawerOpen ? 'rotate-45' : ''}`}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2.5 rounded-lg transition-colors duration-300 ${active ? 'bg-amber-500/10' : 'bg-slate-100 group-hover:bg-slate-200'}`}>
                      {item.icon}
                    </div>
                    <span className={`text-sm font-semibold transition-colors duration-300 ${active ? 'text-amber-600 font-bold' : 'text-slate-600 group-hover:text-[#0f294a]'}`}>
                      {item.label}
                    </span>
                  </div>
                  
                  {/* Chevron Right indicator */}
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className={`w-4 h-4 transition-all duration-300 ${active ? 'text-amber-600 translate-x-0.5' : 'text-slate-400 group-hover:text-[#0f294a] group-hover:translate-x-0.5'}`}>
